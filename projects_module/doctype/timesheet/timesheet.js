...
frappe.ui.form.on("Timesheet", {
	...
	make_invoice: function (frm) {
		...
		dialog.set_primary_action(__("Create Sales Invoice"), () => {
			...
			return frappe.call({
				type: "GET",
				method: "projects.projects_module.doctype.timesheet.timesheet.make_sales_invoice",
				args: {
		...
		if (frm.doc.employee && frm.doc.time_logs) {
			...
			frm.doc.time_logs.forEach((row) => {
				if (row.activity_type) {
					frappe.call({
						method: "projects.projects_module.doctype.timesheet.timesheet.get_activity_cost",
						args: {
