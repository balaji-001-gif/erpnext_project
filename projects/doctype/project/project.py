...
class Project(Document):
	...
	if TYPE_CHECKING:
		...
		from projects.doctype.project_user.project_user import ProjectUser
	...
	def get_billed_amount_from_parent(self):
		"""
		Get billed amount from Sales Invoice where project is set on the parent.
		Note: The project field was removed from SalesInvoice in ERPNext v15.
		Re-add it as a custom field if you need this functionality.
		"""
		try:
			...
			# project column removed from Sales Invoice
			return 0

	def get_billed_amount_from_child(self):
		"""
		Get billed amount from Sales Invoice Item.
		Note: The project field was removed from SalesInvoiceItem in ERPNext v15.
		Re-add it as a custom field if you need this functionality.
		"""
		try:
			total_billed_amount = frappe.db.sql(
			...
			# project column removed from Sales Invoice Item
			return 0
...
def calculate_total_purchase_cost(project: str | None = None):
	"""
	Calculate total purchase cost for a project.
	Note: The project field was removed from PurchaseInvoiceItem in ERPNext v15.
	Re-add it as a custom field if you need per-item project tracking.
	"""
	if project:
		try:
			...
			total_purchase_cost = (
				...
				.where((pi.project == project) & (pi.docstatus == 1))
				.run(as_list=True)
			)
			return total_purchase_cost
		except Exception:
			# project column removed from Purchase Invoice
			return None
	return None
